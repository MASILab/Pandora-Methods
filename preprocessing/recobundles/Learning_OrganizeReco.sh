...
OUTPUTDIR="$MAIN"/learning/Output_Recobundles/"$DATA"_"$SUB"_"$SCAN"
...
for i in "$STREAM_PATH"/streamlines*.nii.gz
do
	echo $i
	INPUT_STREAMLINES="$i"
  ...
  tmp_path="$INPUT_STREAMLINES"_fix.nii.gz
	...
	APPLYTRANSFORM_CMD="antsApplyTransforms -d 3 -i $tmp_path -r $REFERENCE -n Linear -t $ANTS_T12Atlas -t $ANTS_b02T1 -o $OUTPUT_STREAMLINES_NAME"
