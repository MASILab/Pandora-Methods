...
#Skullstrip T1 for MNI
echo -------
echo Skull stripping T1 for MNI registration
T1_STRIPPED_PATH=$JOB_PATH/T1_masked.nii.gz
BET_CMD="bet $T1_PATH/T1.nii.gz $T1_STRIPPED_PATH -f .3 -R -S -B"
echo $BET_CMD
eval $BET_CMD
...
ANTS_CMD="antsRegistrationSyN.sh -d 3 -f $T1_ATLAS_PATH -m $T1_STRIPPED_PATH -o $ANTS_OUT"
echo $ANTS_CMD
eval $ANTS_CMD
