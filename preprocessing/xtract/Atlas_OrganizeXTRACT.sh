...
OUTPUTDIR="$MAIN"/Atlases/XtractLinear/"$DATA"_"$SUB"_"$SCAN"
...
STREAM_PATH="$MAIN"/raw_data/"$DATA"/"$SUB"/"$SCAN"/derivatives/xtract/tracts
...
do
	...
	echo $filename
	OUTPUT_STREAMLINES_NAME="$OUTPUTDIR"/"$filename"/densityNorm.nii.gz
...
OUTPUTDIR="$MAIN"/Atlases/XtractNonlinear/"$DATA"_"$SUB"_"$SCAN"
REFERENCE=/nfs/masi/schilkg1/LearningWM/data/atlases/mni_icbm152_t1_tal_nlin_asym_09c.nii.gz
