...
mkdir $tmp_dir/reg
scp -r hickory:"$MAIN"/raw_data/"$DATA"/"$SUB"/"$SCAN"/xtract/tracts $tmp_dir/
...
STREAM_PATH="$tmp_dir"/tracts

for i in "$STREAM_PATH"/*
do
	echo $i
	INPUT_STREAMLINES="$i"/densityNorm.nii.gz
...
scp -r "$tmp_dir"/Linear/"$DATA"_"$SUB"_"$SCAN" hickory:"$MAIN"/registered_data/XtractLinear
scp -r "$tmp_dir"/Nonlinear/"$DATA"_"$SUB"_"$SCAN" hickory:"$MAIN"/registered_data/XtractNonlinear