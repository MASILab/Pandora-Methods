...
mkdir $tmp_dir/reg
scp -r hickory:"$MAIN"/raw_data/"$DATA"/"$SUB"/"$SCAN"/derivatives/AFQ_clean $tmp_dir/
...
scp -r "$tmp_dir"/Linear/"$DATA"_"$SUB"_"$SCAN" hickory:"$MAIN"/registered_data/AFQcleanLinear
scp -r "$tmp_dir"/Nonlinear/"$DATA"_"$SUB"_"$SCAN" hickory:"$MAIN"/registered_data/AFQcleanNonlinear